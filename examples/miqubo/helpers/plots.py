...
def plot_vars_out(data, vars1, vars2, y, model):
    ...
    ax1.plot(np.linspace(-np.pi, np.pi, 100), model((data[vars1[0]].values, data[vars1[1]].values), *vars1[2]), 'b.')
    ax1.legend(["out", "model"])
    ax2 = plt.subplot(1, 2, 2)
    ax2.set_title("Modeling %s" % vars2[:2])
    ax2.plot(np.linspace(-np.pi, np.pi, 100), y, 'r')
    ax2.plot(np.linspace(-np.pi, np.pi, 100), model((data[vars2[0]].values, data[vars2[1]].values), *vars2[2]), 'b.')
    ax1.legend(["out", "model"])
